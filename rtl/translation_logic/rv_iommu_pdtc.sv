...
module rv_iommu_pdtc #(
    ...
    // Update
    input  rv_iommu::pdtc_up_t      update_i,

    // Lookup
    input  logic                    lookup_i,
    input  rv_iommu::device_id_t    lu_did_i,
    input  rv_iommu::process_id_t   lu_pid_i,
    output rv_iommu::pc_t           lu_content_o,
    ...
    struct packed {
        ...
        logic                   valid;
    } [RVIOMMUCfg.NumPdtcEntries-1:0] tags_q, tags_n;

    // Contents
    rv_iommu::pc_t [RVIOMMUCfg.NumPdtcEntries-1:0] content_q, content_n;

    // Replacement logic
    logic [RVIOMMUCfg.NumPdtcEntries-1:0] lu_hit;
    logic [RVIOMMUCfg.NumPdtcEntries-1:0] replace_en;
    ...
    always_comb begin : lookup
        ...
        if (lookup_i) begin
            ...
            for (int unsigned i = 0; i < RVIOMMUCfg.NumPdtcEntries; i++) begin
        ...
        for (int unsigned i = 0; i < RVIOMMUCfg.NumPdtcEntries; i++) begin
            ...
            */
            if (inval_i.inval_ddtc) begin
                if (tags_q[i].device_id == inval_i.did) begin
                    tags_n[i].valid = 1'b0;
                end
            end
            ...
            */
            else if (inval_i.inval_pdtc) begin
                if (tags_q[i].device_id == inval_i.did && tags_q[i].process_id == inval_i.pid) begin
                    tags_n[i].valid = 1'b0;
                end
            end
            ...
            // normal replacement
                ...
                tags_n[i] = '{
                    ...
                    process_id: update_i.pid,
                    valid:      1'b1
                };

                // update device context
                content_n[i] = update_i.pc;
            end
        end 
    end
    ...
    //-----------------------------------------------
    logic[(RVIOMMUCfg.NumPdtcEntries-2):0] plru_tree_q, plru_tree_n;
    ...
    always_comb begin : plru_replacement
        ...
        for (int unsigned i = 0; i < RVIOMMUCfg.NumPdtcEntries; i++) begin
            ...
            if (lu_hit[i]) begin
                for (int unsigned lvl = 0; lvl < $clog2(RVIOMMUCfg.NumPdtcEntries); lvl++) begin
                    idx_base = $unsigned((2**lvl)-1);
                    shift = $clog2(RVIOMMUCfg.NumPdtcEntries) - lvl;
        ...
        for (int unsigned i = 0; i < RVIOMMUCfg.NumPdtcEntries; i += 1) begin
            automatic logic en;
            automatic int unsigned idx_base, shift, new_index;
            en = 1'b1;
